...
export class ChatroomService {
  ...
  async createOneToOneChatroom(friendId: number, userId: number) {
    /**
    BEGIN
    prisma:query INSERT INTO `chat-room`.`Chatroom` (`id`,`name`,`type`,`createTime`,`updateTime`) VALUES (?,?,?,?,?)
    prisma:query SELECT `chat-room`.`Chatroom`.`id` FROM `chat-room`.`Chatroom` WHERE `chat-room`.`Chatroom`.`id` = ? LIMIT ? OFFSET ?
    prisma:query COMMIT
    prisma:query BEGIN
    prisma:query INSERT INTO `chat-room`.`UserChatroom` (`userId`,`chatroomId`) VALUES (?,?)
    prisma:query SELECT `chat-room`.`UserChatroom`.`userId`, `chat-room`.`UserChatroom`.`chatroomId` FROM `chat-room`.`UserChatroom` WHERE (`chat-room`.`UserChatroom`.`userId` = ? AND `chat-room`.`UserChatroom`.`chatroomId` = ?) LIMIT ? OFFSET ?
    prisma:query COMMIT
    prisma:query BEGIN
    prisma:query INSERT INTO `chat-room`.`UserChatroom` (`userId`,`chatroomId`) VALUES (?,?)
    prisma:query SELECT `chat-room`.`UserChatroom`.`userId`, `chat-room`.`UserChatroom`.`chatroomId` FROM `chat-room`.`UserChatroom` WHERE (`chat-room`.`UserChatroom`.`userId` = ? AND `chat-room`.`UserChatroom`.`chatroomId` = ?) LIMIT ? OFFSET ?
